...
public class SimpleUserView extends LinearLayout {
    ...
    public void update(User user, ImageLoader imageLoader) {
        if (null == user) {
            return;
        }
        ivHeader.setBmobImage(user.getHeader(), BitmapFactory.decodeResource(context.getResources(), R.mipmap.default_user), imageLoader,0,0);
